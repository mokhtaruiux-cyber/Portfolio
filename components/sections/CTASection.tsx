...
export const CTASection = () => {
  ...
  const pacing = sectionPacing.cta;
    ...
    <Section>
      ...
      >
        <div className="flex flex-col items-start text-left gap-6">
          <Reveal delay={pacing.badge} className="inline-flex items-center gap-2 px-4 py-2 rounded-mini border border-accent/20 bg-accent/5">
            <span className="w-2 h-2 rounded-full bg-accent" />
            <span className={cn(typography.labelXs, 'text-accent')}>{siteContent.finalCta.badge}</span>
          </Reveal>
          <SectionTitle
            title={ctaTitleLine}
            highlight={ctaHighlightLine}
            delay={pacing.title}
            stackHighlight
            className={cn('mb-0', darkMode ? 'text-white' : 'text-black')}
          />
